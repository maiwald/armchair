(ns armchair.datascript
  (:require [datascript.core :as d]))

(def schema {:npc/name {:db/unique :db.unique/identity}
             :npc/dialogues {:db/valueType :db.type/ref
                             :db/cardinality :db.cardinality/many
                             :db/isComponent true}
             :location/dialogues {:db/valueType :db.type/ref
                                  :db/cardinality :db.cardinality/many}
             :location/connections {:db/valueType :db.type/ref
                                    :db/cardinality :db.cardinality/many}
...
(d/transact! conn [{:db/id -1
                   ...
                   {:db/id -9
                    :option/index 0
                    :option/text "I could ask you the same."
                    :option/next -11}
                   {:db/id -10
                    :option/index 1
                    :option/text "My name does not matter."
                    :option/next -12}
                   {:db/id -11
                    :line/type :npc
                    :line/text "Great thanks!"
                    :line/npc [:npc/name "Hugo"]
                    :ui/position [609 58]}
                   {:db/id -12
                    :line/type :npc
                    :line/text "Fine, be a jerk!"
                    :line/npc [:npc/name "Hugo"]
                    :ui/position [1205 226]}
                   {:db/id -14
                    :info/description "Hugo's name is Hugo."}
                   {:db/id -15
                    :option/index 2
                    :option/text "Hello Hugo!"
                    :option/next -12
                    :option/req-infos [-14]}
                   ])
